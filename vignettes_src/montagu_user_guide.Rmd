...
```{r}
head(montagu_burden_estimate_sets("IC-Garske", "201710gavi-5", "yf-no-vaccination", location))
```

If we know the burden estimate set id, we can also get a list of information about a burden estimate set with:

```{r}
montagu_burden_estimate_set_info("IC-Garske", "201710gavi-5", "yf-no-vaccination", 687, location)
```

If there are any reported problems with a burden estimate set, we can examine those with

```{r}
montagu_burden_estimate_set_problems("IC-Garske", "201710gavi-5", "yf-no-vaccination", 687, location)
```

and we can retrieve the data for the burden estimate set with:

```{r}
data <- montagu_burden_estimate_set_data("IC-Garske", "201710gavi-5", "yf-no-vaccination", 687, location)
head(data)
```
...
```{r}
head(montagu_model_run_parameter_sets("IC-Garske", "201710gavi-5"))
```

or for a list of information about a single parameter set:

```{r}
montagu_model_run_parameter_set_info("IC-Garske", "201710gavi-5", 20)
```

Finally, if we want to retrieve the parameter values for a previous set:-

```{r}
data <- montagu_model_run_parameter_set_data("IC-Garske", "201710gavi-5", 20)
data[1:5,1:5]
```
